...
import type { Subcontractor, CreateSubcontractorData } from "../types/subcontractor";
import {
    collection,
    addDoc,
    getDocs,
...
export const subcontractorService = {
    ...
    async createSubcontractor(subcontractorData: CreateSubcontractorData): Promise<Subcontractor> {
        const now = new Date();
        ...
        const subcontractorDoc = await addDoc(subcontractorRef, subcontractorToCreate);
    ...
    async getAllSubcontractors(): Promise<Subcontractor[]> {
        const subcontractorRef = collection(db, "subcontractors");
        const subcontractorQuery = query(subcontractorRef, orderBy("createdAt", "desc"));
    ...
    async getSubcontractorsByCity(city: string): Promise<Subcontractor[]> {
        ...
        const subcontractorSnapshot = await getDocs(
            query(subcontractorRef, where("city", "==", city), orderBy("createdAt", "desc"))
        );
        const subcontractors = subcontractorSnapshot.docs.map((doc) => {
            const data = doc.data();
            return {
    ...
    async getSubcontractorsByAvailability(availability: "available" | "busy" | "unavailable"): Promise<Subcontractor[]> {
        const subcontractorRef = collection(db, "subcontractors");
        const subcontractorQuery = query(
            subcontractorRef,
